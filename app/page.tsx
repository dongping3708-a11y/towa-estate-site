...
export default function Home() {
  return (
    <main style={{background:'#070b12', color:'#f7f3ea', fontFamily:'Arial, sans-serif'}}>
      <section style={{minHeight:'100vh', padding:'28px 22px 70px', background:'radial-gradient(circle at 75% 18%, rgba(191,151,91,.28), transparent 35%), linear-gradient(135deg,#05070b 0%,#0b1626 55%,#101827 100%)'}}>
        <header style={{display:'flex', justifyContent:'space-between', alignItems:'center', maxWidth:1120, margin:'0 auto 90px'}}>
        ...
        <div style={{maxWidth:1120, margin:'0 auto', display:'grid', gap:36}}>
          <div style={{maxWidth:760}}>
            <p style={{color:'#c9a46a', letterSpacing:'.22em', fontSize:12, marginBottom:18}}>OSAKA REAL ESTATE / 東和地産</p>
            <h1 style={{fontSize:'clamp(44px, 9vw, 96px)', lineHeight:1.02, margin:0, letterSpacing:'-.05em'}}>Real Estate with Integrity, Care, and Global Vision.</h1>
            <p style={{fontSize:'clamp(18px, 4vw, 25px)', lineHeight:1.65, color:'#d7dce7', maxWidth:680, marginTop:28}}>大阪を拠点に、国内外のお客様へ信頼できる不動産サービスを提供します。購入・売却・管理・投資まで、資産形成の一歩を誠実に支えます。</p>
            <p style={{color:'#bfc7d5', lineHeight:1.8}}>立足大阪，为日本本地客户、中国客户及海外投资者提供专业、安心的不动产服务。</p>
            <div style={{display:'flex', flexWrap:'wrap', gap:14, marginTop:32}}>
              <a href='mailto:info@towaestate.co.jp' style={{background:'#c9a46a', color:'#08101c', padding:'15px 22px', borderRadius:999, textDecoration:'none', fontWeight:700}}>お問い合わせ</a>
          ...
          <div style={{border:'1px solid rgba(255,255,255,.16)', borderRadius:28, padding:22, background:'rgba(255,255,255,.06)', backdropFilter:'blur(10px)'}}>
            <div style={{height:260, borderRadius:20, background:'linear-gradient(135deg, rgba(255,255,255,.12), rgba(201,164,106,.18)), url(https://images.unsplash.com/photo-1590559899731-a382839e5549?auto=format&fit=crop&w=1400&q=80)', backgroundSize:'cover', backgroundPosition:'center'}} />
          </div>
        </div>
      </section>

      <section id='services' style={{padding:'86px 22px', background:'#f6f1e8', color:'#111827'}}>
        <div style={{maxWidth:1120, margin:'0 auto'}}>
          ...
          <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))', gap:18, marginTop:34}}>
            {services.map((s) => (
              <div key={s[0]} style={{background:'#fff', borderRadius:22, padding:24, border:'1px solid #e8dcc8', boxShadow:'0 16px 40px rgba(15,23,42,.08)'}}>
      ...
      <section style={{padding:'88px 22px', background:'#0b1220'}}>
        <div style={{maxWidth:1120, margin:'0 auto', display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))', gap:24, alignItems:'center'}}>
          ...
          <div style={{background:'linear-gradient(145deg,#101a2e,#18233a)', border:'1px solid rgba(255,255,255,.12)', borderRadius:26, padding:28}}>
      ...
      <section style={{padding:'86px 22px', background:'#fff', color:'#111827'}}>
        <div style={{maxWidth:1120, margin:'0 auto'}}>
          ...
          <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(250px,1fr))', gap:18}}>
            {listings.map((item, i)=>(
              <div key={item[0]} style={{borderRadius:24, overflow:'hidden', border:'1px solid #e5e7eb', background:'#fff'}}>
                <div style={{height:190, background:`linear-gradient(rgba(0,0,0,.1),rgba(0,0,0,.35)), url(https://images.unsplash.com/photo-${['1486406146926-c627a92ad1ab','1502672260266-1c1ef2d93688','1494526585095-c41746248156'][i]}?auto=format&fit=crop&w=1000&q=80)`, backgroundSize:'cover', backgroundPosition:'center'}} />
      ...
      <section style={{padding:'86px 22px', background:'#f6f1e8', color:'#111827'}}>
        <div style={{maxWidth:1120, margin:'0 auto', display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))', gap:30}}>
          ...
          <div style={{background:'#111827', color:'#fff', borderRadius:26, padding:28}}>
            <p>〒542-0082</p>
